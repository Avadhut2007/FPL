...
def get_scored_dataframe(force_refresh=False):
    ...
    df = data_processor.build_player_dataframe(
        bootstrap,
        fixtures,
        current_gw
    )
...
def api_squad():
    ...
    try:
        ...
        starting_xi, bench, captain, vice_captain, formation = (
            optimizer.pick_best_starting_xi(squad)
        )

        total_points = float(
            starting_xi["predicted_points"].sum()
            + captain["predicted_points"]
        )
        ...
        return jsonify({
            ...
            "starting_xi": [
                player_to_dict(r)
                for _, r in starting_xi.iterrows()
            ],
            "bench": [
                player_to_dict(r)
...
def api_transfers():
    ...
    try:
        ...
        picks_data = fpl_api.get_entry_picks(
            team_id,
            max(gw - 1, 1)
        )
        ...
        # Auto-pick up "money in the bank" from the entry's own history
        # so the user doesn't have to enter it manually, unless they
        # explicitly did.
        if not budget_bank:
            budget_bank = (
                picks_data
...
def api_team(team_id):
    try:
        ...
        entry_info = fpl_api.get_entry_info(
            team_id
        )
        ...
        squad = data_processor.build_team_squad(
            picks_data,
            bootstrap
        )
        ...
        return jsonify({
            ...
            "overall_points": entry_info.get(
                "summary_overall_points"
            ),
            "overall_rank": entry_info.get(
...
def api_fixtures():
    ...
    try:
        ...
        available_gws = data_processor.get_available_gameweeks(
            fixtures
        )

        gw = (
            requested_gw
            if requested_gw in available_gws
            else current_gw
        )

        items = data_processor.build_fixtures_for_gw(
            bootstrap,
            fixtures,
            gw
        )
...
def api_chart_data():
    ...
    try:
        ...
        return jsonify({
            ...
            "predicted_points": [
                round(float(x), 2)
                for x in top["predicted_points"]
            ],
            "total_points": [
                int(x)
                for x in top["total_points"]
            ],
        })
...
def api_news():
    try:
        ...
        items = data_processor.build_player_news(
            bootstrap
        )
...
def api_history():
    ...
    try:
        ...
        team_lookup = data_processor.build_team_lookup(
            bootstrap
        )
        ...
        for p in top_elements:
            history = fpl_api.get_player_history(
                p["id"]
            )
            ...
            result.append({
                ...
                "position": data_processor.POSITION_MAP.get(
                    p["element_type"],
                    "UNK"
                ),
